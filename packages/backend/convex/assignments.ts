...
export const list = query({
	...
	handler: async (ctx) => {
		...
		const formattedAssignments = await Promise.all(
			assignments.map(async (assignment) => {
				...
				const effectiveDueDate =
					assignment.dueDate ?? now + 7 * 24 * 60 * 60 * 1000;
				...
				return {
					...
					// effective date for ordering/UI; preserve raw and semantics
					dueDate: effectiveDueDate,
					hasActualDueDate: assignment.dueDate != null,
					dueDateOriginal: assignment.dueDate ?? null,
		...
		return formattedAssignments.sort((a, b) => {
			// Items with actual due dates come first, then items without
			if (a.hasActualDueDate && b.hasActualDueDate) {
				return a.dueDate - b.dueDate;
			}
			if (a.hasActualDueDate && !b.hasActualDueDate) return -1;
			if (!a.hasActualDueDate && b.hasActualDueDate) return 1;
			// Both don't have actual due dates - maintain consistent order
			return 0;
		});
	},
});
