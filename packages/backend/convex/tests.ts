    ...
    const formattedTests = await Promise.all(
      tests.map(async (test) => {
        ...
        return {
          ...
          attemptsUsed: testAttempts.filter(a => a.testId === test._id).length,
        }
      })
    )
...
export const getTestById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const attempts = await ctx.db
      .query("testAttempts")
      .withIndex("by_test_user", (q) => q.eq("testId", args.testId).eq("userId", user._id))
      .collect()

    return {
      ...test,
      userAttempts: attempts,
      attemptsRemaining: test.maxAttempts ? test.maxAttempts - attempts.length : null,
    }
  },
})
...
export const createTest = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (user.role !== "instructor" && user.role !== "admin") {
      throw new Error("Unauthorized - Only instructors and admins can create tests")
    }
    ...
    const attemptsWithDetails = await Promise.all(
      testAttempts.map(async (attempt) => {
        ...
        return {
          ...
          totalPoints: test ? test.questions.reduce((sum, q) => sum + q.points, 0) : 0,
...
export const submitAttempt = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const totalPoints = args.answers.reduce((sum, answer) => sum + answer.pointsEarned, 0)
