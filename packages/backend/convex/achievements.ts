...
export const deleteAchievement = mutation({
	...
	handler: async (ctx, args) => {
		...
		await Promise.all(userAchievements.map((ua) => ctx.db.delete(ua._id)));
		...
		if (user) {
			...
			await ctx.db.patch(args.userId, {
				stats: {
					...
					level:
						Math.floor((currentStats.totalXP + achievement.points) / 1000) + 1,
				},
				updatedAt: Date.now(),
			});
		}
				...
				if (user) {
					...
					await ctx.db.patch(args.userId, {
						stats: {
							...
							totalXP: currentStats.totalXP + achievement.points,
							level:
								Math.floor((currentStats.totalXP + achievement.points) / 1000) +
								1,
						},
						updatedAt: Date.now(),
					});
