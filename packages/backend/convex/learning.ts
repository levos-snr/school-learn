...
export const updateProgress = mutation({
  args: {
    ...
    progressType: v.union(v.literal("course"), v.literal("module"), v.literal("lesson")),
...
export const getUserProgress = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.lessonId) {
      const progress = await query.filter((q) => q.eq(q.field("lessonId"), args.lessonId)).unique()
      return progress
    }

    if (args.moduleId) {
      const progress = await query.filter((q) => q.eq(q.field("moduleId"), args.moduleId)).collect()
      return progress
    }
...
export const removeBookmark = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const updatedBookmarks = (progress.bookmarks || []).filter((bookmark) => bookmark.timestamp !== args.timestamp)
...
export const getCourseProgress = query({
  ...
  handler: async (ctx, args) => {
    ...
    const completedLessons = progressRecords.filter((p) => p.progressType === "lesson" && p.isCompleted).length
