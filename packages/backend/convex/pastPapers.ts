	...
	handler: async (ctx, args) => {
		...
		if (args.subject && args.subject !== "all") {
			filteredPapers = filteredPapers.filter(
				(paper) => paper.subject === args.subject,
			);
		}
		if (args.form && args.form !== "all") {
			filteredPapers = filteredPapers.filter(
				(paper) => paper.form === args.form,
			);
		}
		if (args.year) {
			filteredPapers = filteredPapers.filter(
				(paper) => paper.year === args.year,
			);
		}
		if (args.type && args.type !== "all") {
			filteredPapers = filteredPapers.filter(
				(paper) => paper.type === args.type,
			);
		}
		if (args.search) {
...
export const getPastPaperStats = query({
	...
	handler: async (ctx) => {
		...
		const totalDownloads = verifiedPapers.reduce(
...
// Get past papers by course - FIXED VERSION
export const getPastPapersByCourse = query({
	...
	handler: async (ctx, args) => {
		...
		// Safe fallbacks for category and tags with proper normalization
		const courseSubject =
			typeof course.category === "string" 
				? course.category.toLowerCase().trim() 
				: "";
		
		// Pre-normalize course tags and filter out empty strings
		const courseTagsLC = Array.isArray(course.tags) 
			? course.tags
		...
		// Filter papers by subject matching - fixed to handle empty strings properly
		const filteredPapers = papers.filter((paper) => {
			const paperSubject =
				typeof paper.subject === "string" 
					? paper.subject.toLowerCase().trim() 
					: "";

			// Check category match (both must be non-empty)
			const hasCategoryMatch =
				courseSubject.length > 0 &&
				paperSubject.length > 0 &&
				(paperSubject.includes(courseSubject) ||
					courseSubject.includes(paperSubject));

			// Check tag match (tags must exist and be non-empty)
			const hasTagMatch =
				courseTagsLC.length > 0 &&
				paperSubject.length > 0 &&
				courseTagsLC.some(
					(tag) => 
						tag.includes(paperSubject) || 
						paperSubject.includes(tag)
				);

			return hasCategoryMatch || hasTagMatch;
		});

		// PERFORMANCE FIX: Limit to top 10 results BEFORE hydrating uploader details
		const limitedPapers = filteredPapers.slice(0, 10);

		// Get uploader details only for the limited set
		const papersWithUploaders = await Promise.all(
			limitedPapers.map(async (paper) => {
				const uploader = await ctx.db.get(paper.uploaderId);
				return {
					...paper,
					uploaderName: uploader?.name || "Unknown",
