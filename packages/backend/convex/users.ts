...
export const getAllUsers = query({
	args: {
		...
		role: v.optional(
			v.union(v.literal("user"), v.literal("instructor"), v.literal("admin")),
		),
	},
	handler: async (ctx, args) => {
		const identity = await ctx.auth.getUserIdentity();
...
export const updateUserStats = mutation({
	...
	handler: async (ctx, args) => {
		...
		const newStats = {
			...
			level:
				Math.floor((currentStats.xpPoints + (args.xpGained || 0)) / 1000) + 1,
			...
			coursesCompleted:
				currentStats.coursesCompleted + (args.courseCompleted ? 1 : 0),
			assignmentsCompleted:
				currentStats.assignmentsCompleted + (args.assignmentCompleted ? 1 : 0),
			testsCompleted:
				currentStats.testsCompleted + (args.testCompleted ? 1 : 0),
...
export const updateUserSettings = mutation({
	...
	handler: async (ctx, args) => {
		...
		// Authorization - check if user can update these settings
		const isSelf = caller._id === args.userId;
		const isAdmin = caller.role === "admin";
		
		if (!isSelf && !isAdmin) {
			throw new Error("Unauthorized - You can only update your own settings or must be an admin");
		}
		...
		// Deep merge the settings (preserving existing nested values)
