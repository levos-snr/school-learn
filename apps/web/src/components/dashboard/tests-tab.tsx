...
export function TestsTab() {
	...
	const filteredTests = tests?.filter(
		(test) =>
			selectedSubject === "all" ||
			test.courseTitle?.toLowerCase().includes(selectedSubject.toLowerCase()),
	);

	const completedTests =
		attempts?.filter((a) => a.status === "completed").length || 0;
	const averageScore = attempts?.length
		? attempts.reduce((acc, attempt) => acc + attempt.percentage, 0) /
			attempts.length
		: 0;
			...
			<StaggerContainer className="grid gap-4 md:grid-cols-4">
				<FadeIn>
					<Card>
						...
						<CardContent>
							<div className="font-bold text-2xl">{completedTests}</div>
							<p className="text-muted-foreground text-xs">
								Out of {tests?.length || 0} available
							</p>
						</CardContent>
					</Card>
				</FadeIn>
				...
				<FadeIn>
					<Card>
						<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
							<CardTitle className="font-medium text-sm">Best Score</CardTitle>
							<Award className="h-4 w-4 text-muted-foreground" />
						</CardHeader>
						<CardContent>
							<div className="font-bold text-2xl">{Math.round(bestScore)}%</div>
				...
				<FadeIn>
					<Card>
						...
						<CardContent>
							...
							<p className="text-muted-foreground text-xs">Tests due soon</p>
						</CardContent>
					</Card>
				</FadeIn>
			</StaggerContainer>
						...
						{filteredTests
							?.filter((test) => {
								const hasAttempt = attempts?.some(
									(a) => a.testId === test._id && a.status === "completed",
								);
											...
											<CardContent className="space-y-4">
												<div className="grid gap-4 md:grid-cols-3">
													<div className="text-center">
														<div className={`font-bold text-2xl ${scoreColor}`}>
			...
			{filteredTests?.length === 0 && (
				<Card>
					<CardContent className="flex flex-col items-center justify-center py-12">
						...
						<h3 className="mb-2 font-medium text-lg">No tests found</h3>
