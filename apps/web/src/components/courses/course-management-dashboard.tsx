...
export function CourseManagementDashboard({ userRole }: CourseManagementDashboardProps) {
  ...
  const filteredCourses =
    courses?.filter((course) => {
      // keep these checks minimal; underlying type includes isPublished
      // if it doesn't, using 'as any' below avoids errors in UI rendering
      // but filtering still works when present
      const c = course as any
      if (activeTab === "published") return !!c.isPublished
      if (activeTab === "draft") return !c.isPublished
      return true
    }) || []
  ...
  const stats = {
    ...
    draft: courses?.filter((c: any) => !c.isPublished).length || 0,
    enrollments: 0,
  }

  if (showCreateWizard) {
    return <CourseCreationWizard isOpen={true} onClose={() => setShowCreateWizard(false)} />
  }

  if (showBulkUpload) {
    return <BulkCourseUpload isOpen={true} onClose={() => setShowBulkUpload(false)} />
  }
        ...
        <TabsContent value={activeTab} className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCourses.map((course) => {
              const idStr = String((course as any)._id)
              const c = course as any
              const lessonsCount = c.totalLessons ?? c.lessonsCount ?? 0
              const studentsCount = c.totalEnrollments ?? c.enrollments ?? c.studentsCount ?? 0

              return (
                <Card key={idStr} className="group hover:shadow-lg transition-shadow">
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      {(userRole === "admin" || userRole === "instructor") && (
                        <Checkbox
                          checked={selectedCourses.includes(idStr)}
                          onCheckedChange={(checked) => {
                            if (checked === true) {
                              ...
                              setSelectedCourses((prev) => prev.filter((id) => id !== idStr))
                            }
                          }}
                        />
                      )}
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {(userRole === "admin" || userRole === "instructor") && (
                            <>
                              ...
                              <DropdownMenuItem onClick={() => handleResetCourse(idStr)}>
                                <RefreshCw className="h-4 w-4 mr-2" />
                                Reset Course
                              </DropdownMenuItem>
                              <DropdownMenuItem
                                onClick={() => handlePublishCourse(idStr, !c.isPublished)}
                              >
                                {c.isPublished ? (
                                  <>
                                    <Eye className="h-4 w-4 mr-2" />
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-sm font-medium">{`$${c.price ?? 0}`}</p>
                    </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center">
                          <span className="text-xs font-medium text-white">
                            {(c.instructorName?.charAt(0) as string) || "?"}
                          </span>
                        </div>
                        <span className="text-sm font-medium">{c.instructorName ?? "—"}</span>
                      </div>
