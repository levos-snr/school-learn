    ...
    return (
      <FadeIn duration={0.8} className="sticky top-0 z-50 backdrop-blur-md bg-background/80 border-b border-border/50">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 py-3 sm:py-4">
                ...
                <div className="flex items-center space-x-3">
                  <SignInButton>
                    <Button
                      variant="outline"
                      size="sm"
                      className="cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-md hover:bg-accent/20 bg-transparent"
                    >
                      Sign In
                    </Button>
                  </SignInButton>
          ...
          {/* Mobile Menu Overlay */}
          {isMobileMenuOpen && (
            <div className="fixed inset-0 bg-background/80 backdrop-blur-md z-40 md:hidden" onClick={closeMobileMenu}>
              <SlideIn
                direction="down"
                duration={0.4}
                  ...
                  <Unauthenticated>
                    <div className="space-y-3">
                      <SignInButton>
                        <Button
                          variant="outline"
                          className="cursor-pointer w-full justify-center transition-all duration-300 hover:scale-[1.02] bg-transparent"
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 sm:px-6 py-3 gap-4 sm:gap-0">
        ...
        <nav className="flex flex-wrap gap-4 sm:gap-6 text-base sm:text-lg">
          {defaultLinks.map(({ to, label }, index) => (
            <SlideIn key={to} direction="left" duration={0.5} delay={index * 0.1}>
              <Link
                href={to}
